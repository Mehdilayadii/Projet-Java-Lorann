...
public class LorannBDDConnectorTest {
    ...
    public void executeQuery() {
        try {
            ResultSet resultExpected = statement.executeQuery("SELECT element FROM levels");
            ResultSet result = connector.executeQuery("SELECT element FROM levels");
            result.last();
            resultExpected.last();
    ...
    public void prepareCall() {
        try {
            assertNotNull(connector.prepareCall("CALL getLevelByID(1)").executeQuery());
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    public void executeUpdate() {
        assert(connector.executeUpdate("UPDATE levels SET element = 'C' WHERE  element = 'C'") != 0);
    }
