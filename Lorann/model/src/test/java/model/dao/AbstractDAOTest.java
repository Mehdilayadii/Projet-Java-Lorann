...
public class AbstractDAOTest {
    ...
    public void executeQuery() {
        try {
            ResultSet resultExpected = AbstractDAO.executeQuery("SELECT element FROM levels");
            ResultSet result = statement.executeQuery("SELECT element FROM levels");
            result.last();
            resultExpected.last();
    ...
    public void executeUpdate() {
        assert(AbstractDAO.executeUpdate("UPDATE levels SET element = 'C' WHERE  element = 'C'") != 0);
    }
